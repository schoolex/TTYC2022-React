...
class App extends Component {
  ...
  onLetterClick(letter, e) {
    ...
    if (firstIndex !== -1) {
      ...
      for (let i =0; i < letters.length; i++) {
        charArray.push(letters[i].value);
      }
      const s = charArray.join("");
      ...
      this.setState((prevState, props) => {
        return {
          ...
          correctGuesses: gameWon? [...prevState.correctGuesses,s]: prevState.correctGuesses
        };
      });
    } else {
      this.setState((prevState, props) => {
  ...
  onRestartClick(e) {
    e.preventDefault();
    this.setState(gameFactory.restartGame());
  }
}
